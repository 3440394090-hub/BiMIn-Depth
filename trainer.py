...
class Trainer:
    def __init__(self, options):
        ...
        self.models["encoder"] = self.models["encoder"].cuda(1)
        # self.models["encoder"] = torch.nn.DataParallel(self.models["encoder"])
        # self.models["encoder"].to(self.device)
        self.parameters_to_train += list(self.models["encoder"].parameters())


        # bidirectional Mamba query layer(BMQL)
        ...
        self.models["depth"] = self.models["depth"].cuda(1)
        ...
        self.models["pose"] = self.models["pose"].cuda(1)
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            if not self.opt.disable_automasking:
                ...
                weighting_loss = 0.2 * nn.BCELoss()(mask, torch.ones(mask.shape).cuda(1))
            ...
            if not self.opt.disable_automasking:
                ...
                identity_reprojection_loss += torch.randn(
                    identity_reprojection_loss.shape).cuda(1) * 0.00001
