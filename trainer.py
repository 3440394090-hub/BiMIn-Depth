...
class Trainer:
    def __init__(self, options):
        ...
        self.models["encoder"] = networks.RefinedEncoderDecoder(num_layers=self.opt.num_layers, num_features=self.opt.num_features, model_dim=self.opt.model_dim)
        ...
        self.models["encoder"] = self.models["encoder"].cuda(0)
        # self.models["encoder"] = torch.nn.DataParallel(self.models["encoder"])
        # self.models["encoder"].to(self.device)
        self.parameters_to_train += list(self.models["encoder"].parameters())

        self.models["depth"] = networks.BMQL(in_channels=self.opt.model_dim, patch_size=self.opt.patch_size,
                                             dim_out=self.opt.dim_out, embedding_dim=self.opt.model_dim,
                                             query_nums=self.opt.query_nums, num_heads=4, min_val=self.opt.min_depth,
        ...
        self.models["depth"] = self.models["depth"].cuda(0)
        ...
        self.models["pose"] = self.models["pose"].cuda(0)
        ...
        self.model_lr_scheduler = optim.lr_scheduler.StepLR(
            self.model_optimizer, self.opt.scheduler_step_size, self.opt.damp) # default=15
        ...
        for scale in self.opt.scales:
            ...
            if not self.opt.disable_automasking:
                ...
                identity_reprojection_loss += torch.randn(
                    identity_reprojection_loss.shape).cuda(0) * 0.00001
