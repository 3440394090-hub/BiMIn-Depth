...
class ResnetEncoder(nn.Module):
    ...
    def __init__(self, num_layers, pretrained, num_input_images=1):
        ...
        self.ESA_1 = StableESA(n_feats=256)  #spatial context aggregation (SCA)
        self.ESA_2 = StableESA(n_feats=512)  #spatial context aggregation (SCA)
        pretrained_resnet = resnet50(weights="IMAGENET1K_V1")
        self.custom_layer3 = Layer3WithTriplet()  #omni-dimensional residual attention (ODRA)
...
#spatial context aggregation (SCA)
class StableESA(nn.Module):
    def __init__(self, n_feats):
        super(StableESA, self).__init__()
        f = n_feats // 4
...
#omni-dimensional residual attention (ODRA)
...
class Layer2WithTriplet(nn.Module):
    def __init__(self):
        super().__init__()
        # 第1个Bottleneck（stride=2，进行降采样）
        self.block1 = Bottleneck(256, 128, stride=2, downsample=nn.Sequential(
            nn.Conv2d(256, 512, kernel_size=1, stride=2, bias=False),
            nn.BatchNorm2d(512),
        ))
        self.ta1 = TripletAttention(512)  # 降采样后补偿注意力

        # 后续三个 Bottleneck，通道保持512
        self.block2 = Bottleneck(512, 128)
        self.block3 = Bottleneck(512, 128)
        self.ta3 = TripletAttention(512)  # 中层融合增强
        self.block4 = Bottleneck(512, 128)
...
# ---------------- 自定义 layer3 ----------------
class Layer3WithTriplet(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.ta6 = TripletAttention(1024)
        # omni-dimensional residual attention (ODRA)
    def forward(self, x):
